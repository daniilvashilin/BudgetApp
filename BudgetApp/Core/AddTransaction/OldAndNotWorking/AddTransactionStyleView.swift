...
struct AddTransactionStyleView: View {
    ...
    var body: some View {
        VStack {
            ...
            Button("Add +") {
                if let amount = Double(categoryInputAmount) {
                    print("Button tapped: Adding category...")
                    vm.addCategory(name: categoryInputName, description: "some", amount: amount, isMinus: false, image: selectedIcon, imageColro: selectedColor)
                    print("Category added: \(categoryInputName)")
                    isPresented = false
                } else {
                    print("Error: Unable to convert amount to Double")
                }
            }
